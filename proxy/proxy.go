...
func handleConnect(w http.ResponseWriter, r *http.Request) {
	...
	clientReq.Body = pr
	go copyStream(bodyReader, pw, proxy, model.RequestBody)
	...
	targetResp.Body = pr
	go copyStream(remoteBodyReader, pw, proxy, model.ResponseBody)
...
func copyStream(src io.Reader, dst io.Writer, proxy *DataProxy, dataType model.DataType) {
	defer func() {
	...
	const respChunkSize = 8192
	buffer := make([]byte, respChunkSize)
	...
	for {
		...
		if err != nil {
			log.Printf("Error reading HTTPS body: %v", err)
			proxy.reportError(err)
			break
		}
	}
